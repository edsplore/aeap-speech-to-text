...
class CustomProvider extends Writable {
    constructor(options) {
        super(options);
        this.ngrokUrl = options.ngrokUrl || 'https://7979-2405-201-4007-d071-dd18-640e-849b-1778.ngrok-free.app/transcribe';
        this.buffer = [];
        this.bufferSize = 0;
        this.timer = null;
        this.isActive = false;
        this.isFinalizing = false;
        this.MAX_BUFFER_SIZE = options.maxBufferSize || 102400; // Adjust as necessary
        this.MAX_TIME_THRESHOLD = options.maxTimeThreshold || 20000; // Adjust as necessary
    }
    ...
    convertAndSendData(callback) {
        ...
        // Sending buffer directly to FFmpeg's stdin to avoid disk I/O
        const ffmpegProcess = spawn('ffmpeg', ['-f', 'mulaw', '-ar', '8000', '-ac', '1', '-i', 'pipe:0', 'output_audio.wav']);
        ...
        ffmpegProcess.on('close', (code) => {
            if (code !== 0) {
                ...
                this.sendAudioToPython('output_audio.wav', callback);
            }
        });
...
function safeUnlink(path) {
    fs.unlink(path, err => {
        ...
        else console.log(`${path} was deleted successfully.`);
    });
}
