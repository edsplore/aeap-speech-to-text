...
class CustomProvider extends Writable {
    constructor(options) {
        ...
        this.minThresholdSize = 32000; // Minimum threshold in bytes, e.g., 2 seconds of audio at 16kHz, 16-bit PCM
    ...
    _write(chunk, encoding, callback) {
        ...
        if (this.buffer.length >= this.minThresholdSize) {
            if (this.isActive && this.websocket.readyState === WebSocket.OPEN) {
                this.convertAndSendData(this.buffer, callback);
                this.buffer = Buffer.alloc(0); // Clear the buffer after sending
            } else {
                console.log('Waiting for WebSocket connection to be ready...');
                callback();
            }
        } else {
            console.log(`Accumulating data: ${this.buffer.length}/${this.minThresholdSize} bytes`);
            callback();
        }
    }

    convertAndSendData(data, callback) {
        const ffmpegProcess = spawn('ffmpeg', ['-i', 'pipe:0', '-ar', '16000', '-ac', '1', '-f', 'wav', 'pipe:1']);
