...
class GoogleProvider extends Writable {
    ...
    _write(chunk, encoding, callback) {
        // Write the chunk to a temporary raw audio file
        const tmpRaw = tmp.fileSync({ postfix: '.raw' });
        fs.writeFileSync(tmpRaw.name, chunk);
    
        // Convert the raw audio file to the desired format (e.g., MP3) using ffmpeg
        const tmpConverted = tmp.fileSync({ postfix: '.mp3' });
        ffmpeg(tmpRaw.name)
            .output(tmpConverted.name)
            .toFormat('mp3')
            .on('end', () => {
                // Read the converted audio file and send it to the custom API
                const formData = new FormData();
                formData.append('file', fs.createReadStream(tmpConverted.name), { filename: 'audio.mp3' });
    
                axios.post('https://6a16-49-36-184-150.ngrok-free.app/transcribe', formData, {
                    headers: {
                        ...formData.getHeaders(),
                    },
                })
                .then((response) => {
                    ...
                    // Emit a 'result' event with the transcription result
                    this.emit('result', { text: response.data.transcript });
                    callback();
                })
                .catch((error) => {
