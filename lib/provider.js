...
class CustomProvider extends Writable {
    constructor(options) {
        ...
        this.currentlyProcessing = false; // Flag to prevent overlapping processing
    }
    ...
    _write(chunk, encoding, callback) {
        ...
        if (this.queueSize >= this.CHUNK_SIZE && !this.currentlyProcessing) {
    ...
    processData(callback) {
        if (this.queueSize < this.CHUNK_SIZE || this.currentlyProcessing) {
            console.log('Not enough data to process yet or already processing.');
            return callback();
        }
    ...
    sendAudioToPython(filePath, callback) {
        ...
        axios.post(this.ngrokUrl, formData, { headers: formData.getHeaders() })
            .then(response => {
                ...
                const result_for_asterisk = { text: response.data.text, score: Math.round(response.data.confidence * 100) };
                this.emit('result', result_for_asterisk);
                fs.unlink(filePath, err => {
                    if (err) console.error(`Failed to delete ${filePath}: ${err}`);
                });
                this.currentlyProcessing = false;
                this.maybeProcessData(); // Check if there's more data to process
                callback();
            })
            .catch(error => {
