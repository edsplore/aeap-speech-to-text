...
class GoogleProvider extends Writable {
    ...
    _write(chunk, encoding, callback) {
        ...
        // Convert the raw audio to MP3 format
        ...
        ffmpeg(tmpRaw.name)
            ...
            .on('end', () => {
                ...
                fs.readFile(tmpMp3.name, (readErr, mp3Data) => {
                    if (readErr) {
                        console.error('Error reading converted MP3 file:', readErr);
                        callback(readErr);
                        return;
                    }
                    ...
                    formData.append('file', mp3Data, { filename: 'audio.mp3' });
