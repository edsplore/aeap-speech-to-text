...
class GoogleProvider extends Writable {
    ...
    _write(chunk, encoding, callback) {
        ...
        let totalLength = this.audioBuffer.reduce((acc, val) => acc + val.length, 0);
    
        if (totalLength >= this.threshold) {
            let audioData = Buffer.concat(this.audioBuffer);
            this.audioBuffer = []; // Reset the buffer
    
            // Save the raw audio data to a temporary file
            const tempRawFilePath = path.join(__dirname, 'tempRawAudio.raw');
            fs.writeFileSync(tempRawFilePath, audioData);
    
            // Convert the raw audio to a fixed format (e.g., WAV) using ffmpeg
            const tempConvertedFilePath = path.join(__dirname, 'tempConvertedAudio.wav');
            ffmpeg(tempRawFilePath)
                ...
                .on('end', () => {
                    ...
                    formData.append('file', fs.createReadStream(tempConvertedFilePath), { 
                        filename: 'audio.wav', 
                        contentType: 'audio/wav'
                    });
    
                    axios.post('https://1bcf-2405-201-4007-d071-699c-42fb-8c6e-cfd2.ngrok-free.app/transcribe', formData, {
                        headers: {
                            ...formData.getHeaders(),
                        },
                    })
            ...
            callback(); // Proceed to the next chunk if the threshold isn't reached
        }
    }
