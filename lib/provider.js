...
class CustomProvider extends Writable {
    ...
    sendAudioToPython(filePath, callback) {
        ...
        axios.post(this.ngrokUrl, formData, { headers: formData.getHeaders() })
            .then(response => {
                ...
                let result_with_response = await this.sendToChatGPT(response.data.text, callback);
    ...
    sendToChatGPT(transcript, callback) {

        const systemPrompt = "You are a helpful and knowledgeable customer service representative at a well-respected insurance company. Provide clear, supportive, and accurate information in response to the customer's inquiry. Your response should not be more than one small sentence.";
