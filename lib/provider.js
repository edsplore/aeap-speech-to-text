...
class CustomProvider extends Writable {
    ...
    async sendAudioToPython(filePath, callback) {
        ...
        try {
            ...
            // Perform this operation and handle internally
            ...
            callback(null, result_for_asterisk); // Callback should be called once, success case
        } catch (error) {
            console.error('Error sending audio to Python server:', error);
            fs.unlink(filePath, err => {
                if (err) console.error(`Failed to delete ${filePath}:`, err);
            });
            callback(error); // Callback should be called once, error case
        }
    }
