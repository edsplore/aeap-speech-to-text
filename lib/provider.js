...
class CustomProvider extends Writable {
    ...
    async sendToChatGPT(transcript) {
        ...
        try {
            ...
            return audioFilePath;  // Just return the path directly, handle errors in the caller
        } catch (err) {
            console.error('Error in processing ChatGPT response:', err);
    ...
    async sendToLlamaAPI(prompt) {
        this.messages.push({ role: "user", content: prompt });

        try {
            const apiResponse = await axios.post('http://localhost:11434/api/chat', {
                model: "llama3-insurance",
                messages: this.messages,
                stream: false
            });

            let ai_response = apiResponse.data.message.content; 
            console.log('Llama3 API response:', ai_response);
            this.messages.push({ role: "assistant", content: ai_response });
            
            // Assuming there is a function to handle text-to-speech conversion
            const audioFilePath = ai_response;//await this.sendTextToSpeech(ai_response);
            console.log('Audio File Generated:', audioFilePath);
            return audioFilePath;  // Just return the path directly, handle errors in the caller
        } catch (err) {
            console.error('Error in processing Llama3 API response:', err);
            throw err;  // Re-throw the error to be caught by the caller
        }
    }

    async sendAudioToPython(filePath) {
        const formData = new FormData();
        formData.append('file', fs.createReadStream(filePath), { filename: path.basename(filePath) });
    
        try {
            const response = await axios.post(this.ngrokUrl, formData, { headers: formData.getHeaders() });
            console.log('Successfully sent audio data, received response:', response.data);
            
            const result_with_response = await this.sendToLlamaAPI(response.data.text);
            this.bufferResponse({ text: result_with_response, score: Math.round(0.7 * 100) });  // Buffer the response
            fs.unlink(filePath, err => {
                if (err) console.error(`Failed to delete ${filePath}:`, err);
            });
            return result_with_response;
        } catch (error) {
            console.error('Error sending audio to Python server:', error);
            fs.unlink(filePath, err => {
                if (err) console.error(`Failed to delete ${filePath}:`, err);
    ...
    async sendTextToSpeech(text) {
        ...
        try {
            ...
            const audioFilePath = `./audio/${Date.now()}.opus`;
