...
class CustomProvider extends Writable {
    constructor(options, speech) {
        super(options);
        this.speech = speech;  // The speech object passed for sending messages directly
        this.ngrokUrl = options.ngrokUrl || 'http://0.0.0.0:8084/transcribe';
        this.queue = [];
        this.queueSize = 0;
        this.isActive = false;
        this.CHUNK_SIZE = 32000;  // Size of chunks to accumulate before processing
    ...
    sendAudioToPython(filePath, callback) {
        ...
        axios.post(this.ngrokUrl, formData, { headers: formData.getHeaders() })
            .then(response => {
                ...
                this.bufferResponse(result_for_asterisk);
    ...
    bufferResponse(result) {
        ...
        if (!this.bufferTimer) {
            this.bufferTimer = setTimeout(() => this.flushResponseBuffer(), 1000);
        }
    }

    flushResponseBuffer() {
        if (this.responseBuffer.length > 0) {
            this.responseBuffer.forEach(result => {
                this.speech.sendMessage(result); // Here we use the sendMessage method directly
            });
            this.responseBuffer = [];
        }
...
module.exports = {
    getProvider: (name, options, speech) => {
        if (name === "custom") {
            return new CustomProvider(options, speech);
        }
