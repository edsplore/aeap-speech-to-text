...
_write(chunk, encoding, callback) {
   // Assuming the incoming audio chunk is in a format acceptable by your API
   const formData = new FormData();
   formData.append('file', chunk, { filename: 'audio.raw' }); // Adjust filename and format as needed

   axios.post('https://3ff0-2405-201-4007-d071-699c-42fb-8c6e-cfd2.ngrok-free.app/transcribe', formData, {
       headers: {
           ...formData.getHeaders(),
       },
   })
   .then((response) => {
       // Assuming your API returns the transcription result in response.data
       console.log('Transcription result:', response.data);
       // If your implementation requires, process the result here (e.g., emit an event)

       callback(); // Proceed to the next chunk
   })
   .catch((error) => {
       console.error('Error sending audio to custom API:', error);
       callback(error); // Handle error
   });
}
