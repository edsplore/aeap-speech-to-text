...
const ffmpeg = require('fluent-ffmpeg'); // Assuming you're using fluent-ffmpeg
...
class GoogleProvider extends Writable {
    ...
    if (!chunk || chunk.length === 0) {
        ...
        return callback(new Error('Empty chunk received'));
    }

    console.log('Chunk size:', chunk.length); // Log chunk size for debugging
    // Use a temporary file to hold the raw audio data
    const tmpRaw = tmp.fileSync({ postfix: '.raw' });
    fs.writeFileSync(tmpRaw.name, chunk);

    // Convert the raw audio to MP3 format, specifying Mu-law as the input format
    const tmpMp3 = tmp.fileSync({ postfix: '.mp3' });
    ffmpeg(tmpRaw.name)
        .inputFormat('mulaw')
        .output(tmpMp3.name)
        .audioCodec('libmp3lame') // Specify audio codec to be used
        .format('mp3')
        .on('error', (err) => {
            console.error('Error converting file with ffmpeg:', err.message);
            // Cleanup temporary files
            tmpRaw.removeCallback();
        ...
        .on('end', () => {
            ...
            // Check the MP3 file size to ensure it's not empty
            const stats = fs.statSync(tmpMp3.name);
            console.log('Converted MP3 file size:', stats.size);
            if (stats.size === 0) {
                console.error('Conversion resulted in an empty MP3 file');
                callback(new Error('Empty MP3 file generated'));
                return;
            }

            // Prepare form data with the converted MP3 file
            const formData = new FormData();
            formData.append('file', fs.createReadStream(tmpMp3.name), { filename: 'audio.mp3' });

            // Send the MP3 data to the custom API
            axios.post('https://1737-2405-201-4007-d071-699c-42fb-8c6e-cfd2.ngrok-free.app/transcribe', formData, {
                headers: {
                    ...formData.getHeaders(),
                },
            })
            ...
            .catch((error) => {
                console.error('Error sending audio to custom API:', error);
                callback(error);
            })
            .finally(() => {
                // Cleanup temporary files
