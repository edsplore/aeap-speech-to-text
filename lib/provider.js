...
class CustomProvider extends Writable {
    ...
    _write(chunk, encoding, done) {
        ...
        if (this.buffer.length >= this.chunkSize) {
            ...
            this.convertAndSendData(chunkToSend, done);
        } else {
            done();
        }
    }
    ...
    convertAndSendData(chunk, callback) {
        ...
        ffmpegProcess.on('close', (code) => {
            if (code !== 0) {
                ...
                // Audio conversion and transmission complete.
                if (!errorOccurred) {
                    callback();
                }
            }
    ...
    start() {
        if (!this.isActive) {
            // Starting the audio streaming service.
            this.connectWebSocket();
        } else {
    ...
    stop() {
        if (this.isActive) {
            // Stopping the audio streaming service.
            this.isActive = false;
            if (this.websocket) {
                this.websocket.close(1000, 'Normal Closure');
            }
        } else {
            // Stream is already inactive.
        }
    }

    restart() {
        // Restarting the audio streaming service.
        this.stop();
