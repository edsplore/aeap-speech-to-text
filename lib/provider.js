...
class CustomProvider extends Writable {
    constructor(options) {
        ...
        this.websocket = null;
        this.isActive = false;
        this.chunkSize = 90000; // Size of each chunk to handle
        this.buffer = Buffer.alloc(0); // Buffer to accumulate data
    ...
    _write(chunk, encoding, callback) {
        ...
        while (this.buffer.length >= this.chunkSize) {
            ...
            this.convertAndSendData(chunkToSend);
        }
        callback();
    }

    convertAndSendData(chunk) {
        // Convert from µ-law to WAV format using FFmpeg
    ...
    _final(callback) {
        if (this.buffer.length > 0) {
            this.convertAndSendData(this.buffer); // Send any remaining data
