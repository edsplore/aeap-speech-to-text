...
function dispatch(speech) {
    ...
    speech.transport.on("message", (data, isBinary) => {
        ...
        if (msg.hasOwnProperty('request')) {
            let response = handleRequest(speech, msg);
            sendMessage(speech, response);
        } else if (msg.hasOwnProperty('response')) {
            handleResponse(speech, msg); // Assuming this might do something in future
        }
    });

    speech.provider.on("result", (result) => {
        console.log("Result received from provider, sending to client...");
