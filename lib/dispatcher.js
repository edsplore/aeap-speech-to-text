...
function sendMessage(speech, msg) {
	...
	console.log("Text in Dispatcher:", JSON.stringify(msg))
}
...
function dispatch(speech) {
    ...
    speech.transport.on("close", () => {
        console.log("WebSocket connection closed. Ending provider processes.");
        // Properly handle ending the provider without disrupting other components
        if (speech.provider && typeof speech.provider.end === 'function') {
            speech.provider.end();
        } else {
            console.log("Provider does not have an 'end' method or is not set.");
        }
    });
    ...
    // Keep the WebSocket connection active by sending periodic messages if needed
    const keepAliveInterval = setInterval(() => {
        if (speech.transport.readyState === speech.transport.OPEN) {
            sendMessage(speech, { type: "keep-alive", timestamp: new Date() });
        } else {
            clearInterval(keepAliveInterval);
        }
    }, 3000); // Send a keep-alive every 3 seconds
